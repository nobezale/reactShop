...
var listProductInSale=[];

function setListProductInSale(ProductInSaleRes)
{
  listProductInSale= ProductInSaleRes;
}
function getListProductInSale()
{
   return listProductInSale;
}
export default function Album() {
  const classes = useStyles();
  const [data, setData] = useState({productInSale: [], isFetching: false});
 ...
 //  this.setState({message: ''});
  ...
  useEffect(() => {
    const fetchProductInSale = async () => {
        try {
            setData({productInSale: data.productInSaleRes, isFetching: true});
            const response = await fetch('/getAllProductInSale');
            const productInSaleRes = await response.json();
            console.log(productInSaleRes);
            setListProductInSale(productInSaleRes);
            console.log("in useEffect func");
            console.log("List product in sale:");
            console.log( productInSaleRes);
            console.log(typeof(listProductInSale));
            setData({productInSale: productInSaleRes, isFetching: false});
           
        } catch (e) {
            console.log(e);
            setData({productInSale: data.productInSaleRes, isFetching: false});
        }
    };
    fetchProductInSale();
}, []);
      ...
      <main>
        ...
        <div  style={{backgroundImage: "url("+backgroundim+")", height:'240px'} }> 
          <Container  style={{  height:'100%', display: 'flex', alignItems: 'stretch',justifyContent: 'center', flexDirection:'column'}}>
            ...
            amazimg sale! Prices not to be missed!!!
            </Typography>
  ...
  return (
    <React.Fragment>
      ...
      <main>
        ...
        <div className={classes.heroContent}>
        <div  style={{backgroundImage: "url("+backgroundim+")", height:'240px'} }> 
          <Container maxWidth="sm" style={{  height:'100%', display: 'flex', alignItems: 'center',justifyContent: 'center', flexDirection:'column'}} >
            <Typography component="h1" variant="h2" align="center" color="textPrimary"  gutterBottom style={{fontWeight: "bold"}}>
            Sale Summer 2020
            </Typography>
            <Typography variant="h5" align="center" color="textSecondary" paragraph style={{fontWeight: "bold"}}>
